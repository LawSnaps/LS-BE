...
import { BackendRole, BackendRoleSchema } from '@app/schemas/backendRole.schema';
import { BackendRoleRepository } from '@app/repositories/backendRole.repository';


@Module({
    imports: [MongooseModule.forFeature([{ name: BackendRole.name, schema: BackendRoleSchema }])],
    controllers: [BackendRoleController],
    providers: [BackendRoleService,
